...
func Test_HealthController_Ping(t *testing.T) {
	var cases = map[string]struct {
		...
		"should return status up": {
			mocking: func(healthService *mock.MockHealthService) {
				healthService.EXPECT().Ping(gomock.Any()).Return(nil)
			},
		...
		"should return status down": {
			mocking: func(healthService *mock.MockHealthService) {
				healthService.EXPECT().Ping(gomock.Any()).Return(fmt.Errorf("error"))
			},
